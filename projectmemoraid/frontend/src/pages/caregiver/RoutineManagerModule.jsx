...
const RoutineManagerModule = ({ patient, onRefresh }) => {
    ...
    const handleEditRoutine = async (e) => {
        ...
        // Time validation: prevent updating to past times for today
        const now = new Date();
    ...
    const [view, setView] = useState('tasks'); // 'tasks' or 'routines'
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        {dailyTasks.length === 0 ? (
                            <div className="empty-state" style={{ background: '#f8fafc', border: '2px dashed #e2e8f0', padding: '5rem' }}>
                                ...
                                <p style={{ fontWeight: '800', color: '#475569', fontSize: '1.2rem' }}>No routines scheduled today.</p>
                                <p style={{ fontSize: '1rem', color: '#94a3b8', maxWidth: '400px', margin: '0.5rem auto 0' }}>Switch to the 'Routine Builder' to set up the patient's daily care plan.</p>
                            </div>
                        ) : (
                            dailyTasks.map((t) => (
                                ...
                                <div style={{ marginBottom: '1.5rem', flexGrow: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
                                        ...
                                        <span style={{ color: '#64748b', textTransform: 'uppercase', fontWeight: '800', fontSize: '0.75rem', letterSpacing: '1px' }}>
                                            {r.frequency === 'daily' ? 'Every Day' : r.frequency === 'weekly' ? 'Specific Days' : 'Flexible / Custom'}
                                        </span>
                                    </div>
                    ...
                    <div className="admin-modal-overlay" onClick={() => setShowAddModal(false)}>
                        <div className="admin-modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '550px', borderRadius: '32px', padding: '3.5rem' }}>
                            ...
                            <form onSubmit={handleAddRoutine}>
                                ...
                                <div style={{ background: '#f8fafc', padding: '2rem', borderRadius: '24px', marginBottom: '2.5rem', border: '1px solid #f1f5f9' }}>
                                ...
                                <div style={{ display: 'flex', gap: '1.5rem' }}>
                                    <button type="button" className="btn-auth" style={{ background: '#f8fafc', color: '#64748b', border: '1.5px solid #e2e8f0', width: 'auto', padding: '1rem 2.5rem', borderRadius: '18px', fontWeight: '700' }} onClick={() => setShowAddModal(false)}>
                                        Cancel
                                    </button>
                                    <button type="submit" className="btn-auth" style={{ flexGrow: 1, borderRadius: '18px', fontSize: '1.1rem', fontWeight: '800' }}>
                                        Establish Routine
                                    </button>
                                </div>
                            </form>
                    ...
                    <div className="admin-modal-overlay" onClick={() => setShowEditModal(false)}>
                        <div className="admin-modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '550px', borderRadius: '32px', padding: '3.5rem' }}>
                            ...
                            <form onSubmit={handleEditRoutine}>
                                ...
                                <div style={{ display: 'flex', gap: '1.5rem' }}>
                                    <button type="button" className="btn-auth" style={{ background: '#f8fafc', color: '#64748b', border: '1.5px solid #e2e8f0', width: 'auto', padding: '1rem 2.5rem', borderRadius: '18px', fontWeight: '700' }} onClick={() => setShowEditModal(false)}>
                                        Cancel
                                    </button>
                                    <button type="submit" className="btn-auth" style={{ flexGrow: 1, borderRadius: '18px', fontSize: '1.1rem', fontWeight: '800' }}>
                                        Save Changes
